...
OmniParser 可作为 MCP Server 运行，让 Claude Desktop、VS Code Copilot 等 AI 客户端直接调用文档解析能力。
...
| Tool                | 说明                                           |
| ------------------- | ---------------------------------------------- |
| `parse_file`        | 解析单个文件，支持 markdown/json/both 输出格式 |
| `parse_directory`   | 批量解析目录，支持递归和文件数上限             |
| `supported_formats` | 列出所有支持的文件扩展名                       |
| `cache_info`        | 查看缓存状态                                   |
| `cache_clear`       | 清空解析缓存                                   |
...
| URI                              | 说明                         |
| -------------------------------- | ---------------------------- |
| `omniparser://cache/list`        | 列出所有已缓存的解析结果摘要 |
| `omniparser://cache/{file_hash}` | 获取指定 hash 的完整解析结果 |
...
> 💡 如果项目根目录有 `.env` 文件并配置了 `GEMINI_API_KEY`，MCP Server 会自动加载，无需在客户端配置中重复设置。
